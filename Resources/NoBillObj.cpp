...
XMMATRIX	NoBillObj::matBillboard = XMMatrixIdentity();
XMMATRIX	NoBillObj::matBillboardY = XMMatrixIdentity();

/// <summary>
/// 静的メンバ変数の実体
/// </summary>
const float NoBillObj::radius = 5.0f;				// 底面の半径
const float NoBillObj::prizmHeight = 8.0f;			// 柱の高さ
ID3D12Device* NoBillObj::device = nullptr;
UINT NoBillObj::descriptorHandleIncrementSize = 0;
ID3D12GraphicsCommandList* NoBillObj::cmdList = nullptr;
ComPtr<ID3D12RootSignature> NoBillObj::rootsignature;
ComPtr<ID3D12PipelineState> NoBillObj::pipelinestate;
ComPtr<ID3D12DescriptorHeap> NoBillObj::descHeap;
ComPtr<ID3D12Resource> NoBillObj::vertBuff;
//ComPtr<ID3D12Resource> Object3d::indexBuff;
ComPtr<ID3D12Resource> NoBillObj::texbuff;
CD3DX12_CPU_DESCRIPTOR_HANDLE NoBillObj::cpuDescHandleSRV;
CD3DX12_GPU_DESCRIPTOR_HANDLE NoBillObj::gpuDescHandleSRV;
XMMATRIX NoBillObj::matView{};
XMMATRIX NoBillObj::matProjection{};
//XMFLOAT3 Object3d::eye = { 0, 0, -50.0f };
XMFLOAT3 NoBillObj::eye = { 0, 0, -5.0f };
XMFLOAT3 NoBillObj::target = { 0, 0, 0 };
XMFLOAT3 NoBillObj::up = { 0, 1, 0 };
D3D12_VERTEX_BUFFER_VIEW NoBillObj::vbView{};
//D3D12_INDEX_BUFFER_VIEW Object3d::ibView{};
//Object3d::VertexPosNormalUv Object3d::vertices[vertexCount];
NoBillObj::VertexPos NoBillObj::vertices[vertexCount];
//unsigned short Object3d::indices[indexCount];

void NoBillObj::StaticInitialize(ID3D12Device * device, int window_width, int window_height)
{
	// nullptrチェック
	assert(device);

	NoBillObj::device = device;
...
void NoBillObj::PreDraw(ID3D12GraphicsCommandList * cmdList)
{
	// PreDrawとPostDrawがペアで呼ばれていなければエラー
	assert(NoBillObj::cmdList == nullptr);

	// コマンドリストをセット
	NoBillObj::cmdList = cmdList;
...
void NoBillObj::PostDraw()
{
	// コマンドリストを解除
	NoBillObj::cmdList = nullptr;
}

NoBillObj	* NoBillObj::Create()
{
	// 3Dオブジェクトのインスタンスを生成
	NoBillObj* nbObj = new NoBillObj();
	if (nbObj == nullptr) {
		return nullptr;
	}
...
void NoBillObj::SetEye(XMFLOAT3 eye)
{
	NoBillObj::eye = eye;
...
void NoBillObj::SetTarget(XMFLOAT3 target)
{
	NoBillObj::target = target;

	UpdateViewMatrix();
}

void NoBillObj::CameraMoveVector(XMFLOAT3 move)
{
...
void NoBillObj::CameraMoveEyeVector(XMFLOAT3 move)
{
	XMFLOAT3 eye_moved = GetEye();
...
void NoBillObj::InitializeDescriptorHeap()
{
...
void NoBillObj::InitializeCamera(int window_width, int window_height)
{
	// ビュー行列の生成
...
void NoBillObj::InitializeGraphicsPipeline()
{
	HRESULT result = S_FALSE;
...
void NoBillObj::UpdateViewMatrix()
{
	// ビュー行列の更新
...
void NoBillObj::Update()
{
	...
	matTrans = XMMatrixTranslation(-2.0f + position.x, position.y, position.z);
